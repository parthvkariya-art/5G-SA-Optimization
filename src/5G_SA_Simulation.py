...
# ==========================================
# 1. SIMULATION PARAMETERS & PHYSICS (v1.0 Baseline)
# ==========================================
GRID_SIZE = 100         # 100x100 grid (e.g., 10m per cell = 1km x 1km)
P_TX = 40               # High Transmit power in dBm (Macro-cell style)
...
# Define some buildings as (x_min, x_max, y_min, y_max)
BUILDINGS = [
...
def is_blocked(tx_x, tx_y, rx_x, rx_y):
    """Simplified Line-of-Sight check. If receiver is inside a building, it's blocked."""
    for (xmin, xmax, ymin, ymax) in BUILDINGS:
        if xmin <= rx_x <= xmax and ymin <= rx_y <= ymax:
            return True # Simplified: receiver is in a concrete shadow
    return False

def calculate_coverage(towers):
    """Calculates the best signal strength at every point on the grid."""
    coverage_map = np.full((GRID_SIZE, GRID_SIZE), -150.0) # Start with dead zone
    ...
    for tx_x, tx_y in towers:
        for rx_x in range(GRID_SIZE):
            for rx_y in range(GRID_SIZE):
                ...
                # Apply 20-30dB penalty if blocked
                if is_blocked(tx_x, tx_y, rx_x, rx_y):
                    pl += BLOCKAGE_PENALTY
                    
                rx_power = P_TX - pl
                
                # Phone connects to the strongest tower
                if rx_power > coverage_map[rx_x, rx_y]:
                    coverage_map[rx_x, rx_y] = rx_power
                    
    # Calculate percentage of map above -90 dBm
...
def run_simulated_annealing(initial_towers, t_init=1000, cooling_rate=0.90, max_iter=200):
    print("Starting Simulated Annealing (v1.0)...")
    ...
    for i in range(max_iter):
        # PERTURBATION: Move a random tower slightly (No adding/removing yet)
        new_towers = current_towers.copy()
        idx = random.randint(0, len(new_towers) - 1)
        shift_x = random.randint(-10, 10)
        shift_y = random.randint(-10, 10)
        
        # Ensure it stays on the map
        ...
        if delta_J < 0:
            ...
            if random.random() < probability:
                current_towers = new_towers # Accept bad move
                current_cost = new_cost
        ...
        # COOLING
        T *= cooling_rate
        print(f"Iter {i}: Cost = {current_cost:.2f} | Coverage = {new_cov*100:.1f}% | Temp = {T:.2f}")
...
if __name__ == "__main__":
    ...
    # Generate final maps for plotting
